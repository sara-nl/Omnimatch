/**************************************************************************
 * Copyright (C) 2010 W. Baumeister, MPI BioChemistry, Martinsried, Germany
 *
 * This file is part of Omnimatch.
 *
 * Omnimatch is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * Omnimatch is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with Omnimatch.  If not, see <http://www.gnu.org/licenses/>.
 **************************************************************************/

#include <tom.h>
//#define  pi 3.14159265359
#include <stdio.h>
#include <stdlib.h>
#include <math.h>

/* Lowpass filter (reduced complex !) */
void lowpass( sarafft_complex *vol_four, float R, float smooth, int Nx, int Ny, int Nz, sarafft_real scale ) {
  int ikx, iky, ikz, Nx_red, Ny_red, Nz_red;
  float dist, scf = 0.0f;
  sarafft_complex zero;

  Nx_red = ( int ) Nx / 2 + 1;
  Ny_red = ( int ) Ny / 2 + 1;
  Nz_red = ( int ) Nz / 2 + 1;
  c_re(zero) = 0.0; c_im(zero) = 0.0;
  smooth = smooth * smooth; /* use squared smooth for Gaussian */

  for ( ikz = 0; ikz < Nz_red; ikz++ ) {
    for ( iky = 0; iky < Ny_red; iky++ ) {
      for ( ikx = 0; ikx < Nx_red; ikx++ ) {
        /* 1st quadrant */
        dist = ( ( float ) ( ikx * ikx + iky * iky + ikz * ikz ) ) ;
        dist = sqrt( dist );
        if ( dist > R ) {
          if ( smooth > 0.0 ) {
            scf = ( dist - R ) * ( dist - R );
            scf = exp( -scf / smooth );
            c_re(vol_four[ikx + Nx_red*iky + ikz*Nx_red*Ny]) =
              c_re(vol_four[ikx + Nx_red*iky + ikz*Nx_red*Ny]) * scf * scale;
            c_im(vol_four[ikx + Nx_red*iky + ikz*Nx_red*Ny]) =
                c_im(vol_four[ikx + Nx_red*iky + ikz*Nx_red*Ny]) * scf * scale;
          } else {
            vol_four[ikx + Nx_red*iky + ikz*Nx_red*Ny] =  zero ;
          }
        }
        /* other quadrants */
        if ( ( dist > R ) && ( iky > 0 ) ) {
          if ( smooth > 0 ) {
            c_re(vol_four[ikx + Nx_red*( Ny-iky ) + ikz*Nx_red*Ny]) =
                c_re(vol_four[ikx + Nx_red*( Ny-iky ) + ikz*Nx_red*Ny]) * ( double ) scf * scale;
            c_im(vol_four[ikx + Nx_red*( Ny-iky ) + ikz*Nx_red*Ny]) =
                c_im(vol_four[ikx + Nx_red*( Ny-iky ) + ikz*Nx_red*Ny]) * ( double ) scf * scale;
          } else {
            vol_four[ikx + Nx_red*( Ny-iky ) + ikz*Nx_red*Ny] = zero;
          }
        }
        if ( ( dist > R ) && ( ikz > 0 ) ) {
          if ( smooth > 0 ) {
            c_re(vol_four[ikx + Nx_red*iky + ( Nz-ikz )*Nx_red*Ny]) =
                c_re(vol_four[ikx + Nx_red*iky + ( Nz-ikz )*Nx_red*Ny]) * ( double ) scf * scale;
            c_im(vol_four[ikx + Nx_red*iky + ( Nz-ikz )*Nx_red*Ny]) =
                c_im(vol_four[ikx + Nx_red*iky + ( Nz-ikz )*Nx_red*Ny]) * ( double ) scf * scale;
          } else {
            vol_four[ikx + Nx_red*iky + ( Nz-ikz )*Nx_red*Ny] = zero;
          }
        }
        if ( ( dist > R ) && ( iky > 0 ) && ( ikz > 0 ) ) {
          if ( smooth > 0 ) {
            c_re(vol_four[ikx + Nx_red*( Ny-iky ) + ( Nz-ikz )*Nx_red*Ny]) =
                c_re(vol_four[ikx + Nx_red*( Ny-iky ) + ( Nz-ikz )*Nx_red*Ny]) * ( double ) scf * scale;
            c_im(vol_four[ikx + Nx_red*( Ny-iky ) + ( Nz-ikz )*Nx_red*Ny]) =
                c_im(vol_four[ikx + Nx_red*( Ny-iky ) + ( Nz-ikz )*Nx_red*Ny]) * ( double ) scf * scale;
          } else {
            vol_four[ikx + Nx_red*( Ny-iky ) + ( Nz-ikz )*Nx_red*Ny] = zero;
          }
        }
      }
    }
  }
}

/* Bandpass - has lower and upper restrictions   */
void bandpass( sarafft_complex *vol_four, float R_down, float R_up, float smooth, int Nx, int Ny, int Nz, sarafft_real scale ) {
  int ikx, iky, ikz, Nx_red, Ny_red, Nz_red;
  float dist = 0.0f, scf = 0.0f;
  sarafft_complex zero;

  Nx_red = ( int ) Nx / 2 + 1;
  Ny_red = ( int ) Ny / 2 + 1;
  Nz_red = ( int ) Nz / 2 + 1;
  c_re(zero) = 0.0; c_im(zero) = 0.0;
  smooth = smooth * smooth; /* use squared smooth for Gaussian */

  for ( ikz = 0; ikz < Nz_red; ikz++ ) {
    for ( iky = 0; iky < Ny_red; iky++ ) {
      for ( ikx = 0; ikx < Nx_red; ikx++ ) {
        dist = ( ( float ) ( ikx * ikx + iky * iky + ikz * ikz ) ) ;
        dist = sqrt( dist );
        /*   ------ LOWPASS ------  */
        /* 1st quadrant */
        if ( dist > R_up ) {
          if ( smooth > 0.0 ) {
            scf = ( dist - R_up ) * ( dist - R_up );
            scf = exp( -scf / smooth );
            c_re(vol_four[ikx + Nx_red*iky + ikz*Nx_red*Ny]) =
                c_re(vol_four[ikx + Nx_red*iky + ikz*Nx_red*Ny]) * scf * scale;
            c_im(vol_four[ikx + Nx_red*iky + ikz*Nx_red*Ny]) =
                c_im(vol_four[ikx + Nx_red*iky + ikz*Nx_red*Ny]) * scf * scale;
          } else {
            vol_four[ikx + Nx_red*iky + ikz*Nx_red*Ny] =  zero ;
          }
        }
        /* other quadrants */
        if ( ( dist > R_up ) && ( iky > 0 ) ) {
          if ( smooth > 0 ) {
            c_re(vol_four[ikx + Nx_red*( Ny-iky ) + ikz*Nx_red*Ny]) =
                c_re(vol_four[ikx + Nx_red*( Ny-iky ) + ikz*Nx_red*Ny]) * ( double ) scf * scale;
            c_im(vol_four[ikx + Nx_red*( Ny-iky ) + ikz*Nx_red*Ny]) =
                c_im(vol_four[ikx + Nx_red*( Ny-iky ) + ikz*Nx_red*Ny]) * ( double ) scf * scale;
          } else {
            vol_four[ikx + Nx_red*( Ny-iky ) + ikz*Nx_red*Ny] = zero;
          }
        }
        if ( ( dist > R_up ) && ( ikz > 0 ) ) {
          if ( smooth > 0 ) {
            c_re(vol_four[ikx + Nx_red*iky + ( Nz-ikz )*Nx_red*Ny]) =
                c_re(vol_four[ikx + Nx_red*iky + ( Nz-ikz )*Nx_red*Ny]) * ( double ) scf * scale;
            c_im(vol_four[ikx + Nx_red*iky + ( Nz-ikz )*Nx_red*Ny]) =
                c_im(vol_four[ikx + Nx_red*iky + ( Nz-ikz )*Nx_red*Ny]) * ( double ) scf * scale;
          } else {
            vol_four[ikx + Nx_red*iky + ( Nz-ikz )*Nx_red*Ny] = zero;
          }
        }
        if ( ( dist > R_up ) && ( iky > 0 ) && ( ikz > 0 ) ) {
          if ( smooth > 0 ) {
            c_re(vol_four[ikx + Nx_red*( Ny-iky ) + ( Nz-ikz )*Nx_red*Ny]) =
                c_re(vol_four[ikx + Nx_red*( Ny-iky ) + ( Nz-ikz )*Nx_red*Ny]) * ( double ) scf;
            c_im(vol_four[ikx + Nx_red*( Ny-iky ) + ( Nz-ikz )*Nx_red*Ny]) =
                c_im(vol_four[ikx + Nx_red*( Ny-iky ) + ( Nz-ikz )*Nx_red*Ny]) * ( double ) scf;
          } else {
            vol_four[ikx + Nx_red*( Ny-iky ) + ( Nz-ikz )*Nx_red*Ny] = zero;
          }
        }
        /*  ---- HIPASS ----   */
        /* 1st quadrant */
        if ( dist < R_down ) {
          if ( smooth > 0.0 ) {
            scf = ( dist - R_down ) * ( dist - R_down );
            scf = exp( -scf / smooth );
            c_re(vol_four[ikx + Nx_red*iky + ikz*Nx_red*Ny]) =
                c_re(vol_four[ikx + Nx_red*iky + ikz*Nx_red*Ny]) * scf * scale;
            c_im(vol_four[ikx + Nx_red*iky + ikz*Nx_red*Ny]) =
                c_im(vol_four[ikx + Nx_red*iky + ikz*Nx_red*Ny]) * scf * scale;
          } else {
            vol_four[ikx + Nx_red*iky + ikz*Nx_red*Ny] =  zero ;
          }
        }
        /* other quadrants */
        if ( ( dist < R_down ) && ( iky > 0 ) ) {
          if ( smooth > 0 ) {
            c_re(vol_four[ikx + Nx_red*( Ny-iky ) + ikz*Nx_red*Ny]) =
                c_re(vol_four[ikx + Nx_red*( Ny-iky ) + ikz*Nx_red*Ny]) * ( double ) scf * scale;
            c_im(vol_four[ikx + Nx_red*( Ny-iky ) + ikz*Nx_red*Ny]) =
                c_im(vol_four[ikx + Nx_red*( Ny-iky ) + ikz*Nx_red*Ny]) * ( double ) scf * scale;
          } else {
            vol_four[ikx + Nx_red*( Ny-iky ) + ikz*Nx_red*Ny] = zero;
          }
        }
        if ( ( dist < R_down ) && ( ikz > 0 ) ) {
          if ( smooth > 0 ) {
            c_re(vol_four[ikx + Nx_red*iky + ( Nz-ikz )*Nx_red*Ny]) =
                c_re(vol_four[ikx + Nx_red*iky + ( Nz-ikz )*Nx_red*Ny]) * ( double ) scf * scale;
            c_im(vol_four[ikx + Nx_red*iky + ( Nz-ikz )*Nx_red*Ny]) =
                c_im(vol_four[ikx + Nx_red*iky + ( Nz-ikz )*Nx_red*Ny]) * ( double ) scf * scale;
          } else {
            vol_four[ikx + Nx_red*iky + ( Nz-ikz )*Nx_red*Ny] = zero;
          }
        }
        if ( ( dist < R_down ) && ( iky > 0 ) && ( ikz > 0 ) ) {
          if ( smooth > 0 ) {
            c_re(vol_four[ikx + Nx_red*( Ny-iky ) + ( Nz-ikz )*Nx_red*Ny]) =
                c_re(vol_four[ikx + Nx_red*( Ny-iky ) + ( Nz-ikz )*Nx_red*Ny]) * ( double ) scf * scale;
            c_im(vol_four[ikx + Nx_red*( Ny-iky ) + ( Nz-ikz )*Nx_red*Ny]) =
                c_im(vol_four[ikx + Nx_red*( Ny-iky ) + ( Nz-ikz )*Nx_red*Ny]) * ( double ) scf * scale;
          } else {
            vol_four[ikx + Nx_red*( Ny-iky ) + ( Nz-ikz )*Nx_red*Ny] = zero;
          }
        }
      }
    }
  }
}

void correl( sarafft_complex *vol_four, sarafft_complex *ref_four, int Nx, int Ny, int Nz, sarafft_real scale ) {
  int kx, ky, kz, Nx_red, irun, ijk;
  sarafft_real Ctmp, Ctmpim;

  Nx_red = Nx / 2 + 1;
  irun = 0;
  for ( kz = 0; kz < Nz; kz++ ) {
    for ( ky = 0; ky < Ny; ky++ ) {
      for ( kx = 0; kx < Nx_red; kx++ ) {
        ijk = kx + Nx_red * ( ky + kz * Ny );
        Ctmp = c_re(ref_four[ijk]);
        Ctmpim = c_im(ref_four[ijk]);
        c_re(ref_four[ijk]) = ( c_re(ref_four[ijk]) * c_re(vol_four[ijk]) + c_im(ref_four[ijk]) * c_im(vol_four[ijk]) ) * scale;
        c_im(ref_four[ijk]) = ( Ctmp * c_im(vol_four[ijk]) - Ctmpim * c_re(vol_four[ijk]) ) * scale;
        irun++;
      }
    }
  }
}

void convolve( sarafft_complex *vol_four, sarafft_complex *ref_four, int Nx, int Ny, int Nz, sarafft_real scale )
/* output of convolution is written to ref_four */
{
  int kx, ky, kz, Nx_red, irun, ijk;
  sarafft_real Ctmp, Ctmpim;

  Nx_red = Nx / 2 + 1;
  irun = 0;
  for ( kz = 0; kz < Nz; kz++ ) {
    for ( ky = 0; ky < Ny; ky++ ) {
      for ( kx = 0; kx < Nx_red; kx++ ) {
        ijk = kx + Nx_red * ( ky + kz * Ny );
        Ctmp = c_re(ref_four[ijk]);
        Ctmpim = c_im(ref_four[ijk]);
        c_re(ref_four[ijk]) = ( c_re(ref_four[ijk]) * c_re(vol_four[ijk]) - c_im(ref_four[ijk]) * c_im(vol_four[ijk]) ) * scale;
        c_im(ref_four[ijk]) = ( Ctmp * c_im(vol_four[ijk]) + Ctmpim * c_re(vol_four[ijk]) ) * scale;
        irun++;
      }
    }
  }
}
