	      ...
	      for (laufx=0;laufx<Ox_max;laufx++)
		{
		  ...
		  for (laufmx=1;laufmx<=3;laufmx++)
		      ...
		      for(laufmy=1;laufmy<=3;laufmy++)
			{
		      ...
		      if (New_pt[laufmx]<0.0 || New_pt[1]>(float)Ox_max-1.0 || 
			  New_pt[2]>(float)Oy_max-1.0  || New_pt[3]>(float)Oz_max-1.0)
			{
		  ...
		  if (Flag<1)
		    {
		      ...
		      if ( (lauf+Lay_xy+Ox_max+1 < Ox_max*Oy_max*Oy_max) && 
			   (lauf+Ox_max+1 < Ox_max*Oy_max*Oy_max) && (lauf+Lay_xy+1 < Ox_max*Oy_max*Oy_max) 
			   && (lauf+Lay_xy+1 < Ox_max*Oy_max*Oy_max )  )
			{
